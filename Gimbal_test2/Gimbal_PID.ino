float gimbalYawPID(float ang_ref){
  float yaw_kp = 2.5;
  float yaw_ki = 2.0; //1.5
  float yaw_kd = 0.2;
  ...
  if(yaw_ang_diff > -0.1 && yaw_ang_diff < 0.1){
    ...
    yawP = yaw_kp*yaw_cnt_diff;
    yawI += yaw_ki*yaw_cnt_diff*dt;
    yawD = yaw_kd*(yaw_cnt_diff - prev_yaw_cnt_diff)/dt;
/*
...
float gimbalPitchPID(float ang_ref){
  float pitch_kp = 1.0; //80-0-0 //2
  float pitch_ki = 2.0;
  float pitch_kd = 0.4; //2
  int cnt_ref = (int)ang_ref*8191.0/360;
...
//in case going beyond the gap between 0 and 360
  ...
  if(pitch_ang_diff > -0.1 && pitch_ang_diff < 0.1){
    ...
    pitchP = pitch_kp*pitch_cnt_diff;//-
    pitchI += pitch_ki*pitch_cnt_diff*dt;
    pitchD = pitch_kd*(pitch_cnt_diff - prev_pitch_cnt_diff)/dt;
    //yaw_ctrl = -yaw_ang_diff*ctrl_scale; //**
    /*
